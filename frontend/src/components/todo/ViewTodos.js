...
function Todos({isAuthenticated, setIsAuthenticated}) {
	...
	useEffect(() => {
		const loadData = async () => {
			let response = null;
			try {
				let url = `http://13.60.58.245:3001/api/todo/${pageNumber - 1}/${pageSize}`;

				if(filter === 'Completed'){
					url = `http://13.60.58.245:3001/api/todo/${pageNumber - 1}/${pageSize}?isCompleted=true`;
				} else if(filter === 'Not Completed'){
					url = `http://13.60.58.245:3001/api/todo/${pageNumber - 1}/${pageSize}?isCompleted=false`;
				}
	...
	const markCompleted = async (id) => {
		try {
      await axios.put(`http://13.60.58.245:3001/api/todo/${id}/markcomplete`, {}, {
				headers: {
	...
	const deleteTodo = async (id) => {
		try {
      await axios.delete(`http://13.60.58.245:3001/api/todo/${id}`, {
				headers: {
					'Authorization': `Bearer ${sessionStorage.getItem('token')}`
				}
			});
